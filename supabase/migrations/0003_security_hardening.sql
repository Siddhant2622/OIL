...
-- 1. Restrict profiles self-update: prevent role, org_id, manager_id escalation
...
begin
  ...
  if (auth.uid() = old.id and auth.role() = 'authenticated') then
    ...
    if (new.manager_id is distinct from old.manager_id and old.manager_id is not null) then
      raise exception 'Unauthorized: Cannot change manager.';
    end if;
    if (new.is_active is distinct from old.is_active) then
      raise exception 'Unauthorized: Cannot change active status.';
    end if;
  end if;
  return new;
end;
